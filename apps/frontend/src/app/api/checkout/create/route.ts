...
import {
  transformShopifyError,
  extractAndTransformUserErrors,
} from "@/lib/utils/shopify-error-handler";
import {
  normalizeDataForAPI,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (cartResponse?.cartCreate) {
      ...
      if (userError) {
        console.error(
          "❌ UserError création panier Shopify:",
    ...
    try {
      ...
      const existingCustomer = customersData?.customers?.find(
        (c: any) =>
          c.email?.toLowerCase() === normalizedShippingInfo.email.toLowerCase(),
      );
    ...
    const draftOrderDataRaw = {
      ...
      shipping_address: {
        ...
        address2: normalizedShippingInfo.address2
          ? normalizeDataForAPI(normalizedShippingInfo.address2)
          : undefined,
        city: normalizeDataForAPI(normalizedShippingInfo.city),
        zip: normalizeDataForAPI(normalizedShippingInfo.postalCode),
        country: normalizeDataForAPI(
          normalizedShippingInfo.country || "France",
        ),
        phone: normalizedShippingInfo.phone
          ? normalizeDataForAPI(normalizedShippingInfo.phone)
          : undefined,
      },
      shipping_line: {
        title: normalizeDataForAPI(
      ...
      note: normalizeDataForAPI(
        `Checkout personnalise - ${shippingMethod.type === "express" ? "Express" : "Standard"}`,
      ),
    };

    const finalDraftOrder = normalizeDataForAPI(draftOrderDataRaw);
    const draftOrderResponse =
      await adminClient.createDraftOrder(finalDraftOrder);

    if (draftOrderResponse.errors) {
      console.error(
        "❌ Erreur création draft order:",
        JSON.stringify(draftOrderResponse.errors, null, 2),
      );
      return NextResponse.json(
        {
          error:
            draftOrderResponse.errors[0]?.message ||
    ...
    return NextResponse.json(
      { error: message, traceId: `checkout-create-${Date.now()}` },
      { status: 500 },
    );
  }
}
