...
async function getShopifyAdminClient() {
  ...
  // Le script est dans apps/frontend/scripts/, donc le chemin relatif est correct
  const { getShopifyAdminClient } =
    await import("../app/src/lib/ShopifyAdminClient");
...
async function main() {
  ...
  try {
    console.log(
      "🚀 Démarrage de l'envoi d'invitations de réinitialisation de mot de passe...\n",
    );

    // Vérifier que SHOPIFY_ADMIN_TOKEN est configuré
    if (!process.env.SHOPIFY_ADMIN_TOKEN) {
      console.error(
        "❌ SHOPIFY_ADMIN_TOKEN n'est pas configuré dans les variables d'environnement",
      );
      process.exit(1);
    }
    ...
    console.log(
      `📋 Recherche des utilisateurs avec shopifyCustomerId (limite: ${limit})...`,
    );
    ...
    for (const user of users) {
      ...
      try {
        console.log(
          `📧 Envoi invitation à ${user.email} (Customer ID: ${user.shopifyCustomerId})...`,
        );
        ...
        if (isDryRun) {
          ...
          const inviteResult =
            await adminClient.sendCustomerPasswordResetInvite(
          ...
          if (inviteResult.errors && inviteResult.errors.length > 0) {
            console.error(
              `   ❌ Erreur:`,
              inviteResult.errors[0]?.message || "Erreur inconnue",
            );
            errorCount++;
          } else {
    ...
    if (isDryRun) {
      console.log(
        "💡 Pour envoyer réellement les invitations, exécutez sans --dry-run",
      );
    }
  } catch (error) {
