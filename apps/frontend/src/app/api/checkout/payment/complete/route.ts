...
function validatePayPalData(data: PaymentCompleteData): {
  valid: boolean;
  error?: string;
} {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const normalizedPaymentGateway = paymentGateway
      ? normalizeDataForAPI(paymentGateway)
      : undefined;
    const normalizedTransactionId = transactionId
      ? normalizeDataForAPI(transactionId)
      : paypalOrderID
        ? normalizeDataForAPI(paypalOrderID)
        : undefined;
    ...
    if (completedOrder.order_id) {
      const orderResponse = await adminClient.getOrder(
    ...
    return NextResponse.json(
      { error: message, traceId: `payment-complete-${Date.now()}` },
      { status: 500 },
    );
  }
}
