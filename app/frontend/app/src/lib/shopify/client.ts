...
function getShopifyConfig(): { endpoint: string; token: string } | null {
  try {
    ...
    console.error('❌ Erreur de configuration Shopify:', error instanceof Error ? error.message : 'Variables d\'environnement non configurées');
    return null;
  }
}
  ...
  try {
    ...
    if (ENV.NODE_ENV === 'development') {
      const scanForUnicode = (str: string, name: string): void => {
        for (let i = 0; i < str.length; i++) {
          ...
          if (code > 255) {
            ...
            console.error(`❌ Caractère Unicode détecté dans ${name} à l'index ${i}: "${str[i]}" (code: ${code}, U+${code.toString(16).toUpperCase().padStart(4, '0')})`);
      ...
      if (normalizedVariables) {
        scanForUnicode(JSON.stringify(normalizedVariables), 'variables normalisées');
      }
    }
    ...
    while (iteration < maxIterations) {
      ...
      for (let i = 0; i < body.length; i++) {
        ...
        if (code > 255) {
          ...
          console.error(`❌ ERREUR CRITIQUE dans shopifyFetch (itération ${iteration + 1}): Caractère > 255 toujours présent dans le body à l'index ${i}: "${body[i]}" (code: ${code})`);
          console.error(`   Contexte: "${context}"`);
    ...
    if (iteration >= maxIterations) {
      console.error(`❌ ERREUR CRITIQUE: Impossible de nettoyer complètement le body après ${maxIterations} itérations`);
    ...
    for (let i = 0; i < body.length; i++) {
      const code = body.charCodeAt(i);
      if (code > 255) {
        console.error(`❌ ERREUR ABSOLUE dans shopifyFetch: Caractère > 255 toujours présent dans le body à l'index ${i} après toutes les tentatives`);
    ...
    if (ENV.NODE_ENV === 'development') {
      const scanForUnicode = (str: string, name: string): void => {
        for (let i = 0; i < str.length; i++) {
          const code = str.charCodeAt(i);
          if (code > 255) {
            console.error(`❌ Caractère Unicode détecté dans ${name} à l'index ${i}: "${str[i]}" (code: ${code})`);
          }
        }
      };
    ...
    if (ENV.NODE_ENV === 'development') {
      Object.entries(headers).forEach(([key, value]) => {
        for (let i = 0; i < value.length; i++) {
          ...
          if (code > 255) {
            console.error(`❌ Caractère Unicode détecté dans header "${key}" à l'index ${i}: "${value[i]}" (code: ${code})`);
          }
        }
      });
    }
    ...
    if (!result.ok) {
      ...
      if (errorText.includes('ByteString') || errorText.includes('character at index')) {
        ...
        return { 
          data: {} as T,
          errors: [{ 
            message: 'Erreur d\'encodage Unicode dans la requête. Vérifiez les caractères spéciaux dans les données.' 
          }] 
        };
      }
    ...
    if (ENV.NODE_ENV === 'development' && jsonResponse.errors) {
      console.warn('⚠️ Erreurs GraphQL dans la réponse Shopify:', JSON.stringify(jsonResponse.errors, null, 2));
    }
