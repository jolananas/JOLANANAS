...
import { getCartIdFromRequest } from "@/lib/utils/cart-storage";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [customerResult, addressesResult, ordersResult, cartId] =
      await Promise.all([
    ...
    return NextResponse.json({
      ...
      dashboard: {
        user: {
          ...
          emailVerified: customer.createdAt
        ...
        charts: {
          ordersByMonth: Object.entries(ordersByMonth).map(
