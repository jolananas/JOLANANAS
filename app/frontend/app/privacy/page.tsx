...
export const metadata: Metadata = {
  ...
  description: 'Politique de confidentialité de JOLANANAS. Découvrez comment nous collectons, utilisons et protégeons vos données personnelles conformément au RGPD.',
  robots: {
    index: true,
    follow: true,
  },
  openGraph: {
    title: 'Politique de Confidentialité - JOLANANAS',
    description: 'Politique de confidentialité de JOLANANAS',
    type: 'website',
  },
};
...
export default async function PrivacyPage() {
  ...
  const filePath = join(process.cwd(), 'public/assets/documents/Confidentialité — JOLANANAS.md');
  
  let fileContent: string;
  try {
    fileContent = await readFileWithUnicode(filePath);
  } catch (error) {
    console.error('❌ Erreur lors de la lecture du fichier Confidentialité:', error);
    throw new Error('Impossible de charger la Politique de Confidentialité. Veuillez réessayer plus tard.');
  }
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Confidentialité</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
