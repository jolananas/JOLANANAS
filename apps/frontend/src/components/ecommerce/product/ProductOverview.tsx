...
export function ProductOverview({
  ...
  useEffect(() => {
    if (product.variants && product.variants.length > 0 && product.options) {
      ...
      product.options.forEach((option) => {
        const optionValue = firstVariant.selectedOptions.find(
      ...
      return product.variants.find((variant) => {
        return product.options?.every((option) => {
          ...
          return variant.selectedOptions.some(
            (so) => so.name === option.name && so.value === selectedValue,
          );
        });
      });
    },
  ...
  const isVariantAvailable =
    currentVariant?.availableForSale ?? product.availableForSale;
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <Badge
                    variant="default"
                    className="text-2xl px-0 py-0 bg-transparent text-jolananas-pink-deep font-bold"
                  >
          ...
          <CardContent className="space-y-6">
            ...
            {!isVariantAvailable && (
              <Alert variant="destructive" role="alert">
                <AlertDescription>
                  {currentVariant
                    ? "Cette variante est actuellement épuisée. Veuillez sélectionner une autre variante."
                    : "Ce produit est actuellement épuisé. Nous vous informerons dès qu'il sera à nouveau disponible."}
                </AlertDescription>
              </Alert>
            )}
              ...
              product.variants.length > 0 && (
                <div className="space-y-4">
                  {product.options.map((option) => {
                    ...
                    const displayName =
                      option.name.length <= 2 && option.name.trim() !== ""
                        ? "Variantes"
                        : option.name;
                      ...
                      <div key={option.name} className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {option.values.map((value) => {
                            ...
                            const variantExists =
                              findVariantByOptions(tempOptions) !== null;
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <Button
                    ...
                    disabled={
                      !isVariantAvailable || isAddingToCart || !selectedVariant
                    }
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      <Heart
                        className={`h-4 w-4 mr-2 ${isFavorite ? "fill-jolananas-pink-medium text-jolananas-pink-medium" : ""}`}
                      />
              ...
              <TabsContent value="shipping" className="mt-4">
                {shippingInfo ? (
                  <div className="space-y-2 text-sm">
                    <p>
                      <strong>Livraison gratuite</strong> à partir de{" "}
                      {shippingInfo.freeShippingThreshold}€ d'achat.
                    </p>
                    <p>
                      Délai de livraison :{" "}
                      <strong>{shippingInfo.deliveryDaysFrance}</strong> en
                      France métropolitaine.
                    </p>
                    <p>{shippingInfo.deliveryDaysInternational}</p>
                  </div>
                ) : (
                  <Alert
                    variant="default"
                  ...
                  >
                    <AlertDescription className="text-blue-800 dark:text-blue-200">
                      ...
                      <br />
                      Les frais de livraison, la TVA et le total final seront
                      calculés lors du paiement sécurisé selon votre adresse de
                      livraison.
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <AccordionContent>
                  <div className="space-y-4">
                    <div>
                      ...
                      {shippingInfo ? (
                        <p className="text-sm text-muted-foreground">
                          Livraison gratuite à partir de{" "}
                          {shippingInfo.freeShippingThreshold}€. Délai de
                          livraison : {shippingInfo.deliveryDaysFrance} en
                          France métropolitaine.
                        </p>
                      ) : (
                        <p className="text-sm text-muted-foreground">
                          Les informations de livraison ne sont pas disponibles
                          pour le moment.
                        </p>
                      )}
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Oui, vous pouvez retourner ce produit sous 30 jours
                        après réception, sans justification.
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Tous nos produits bénéficient de notre garantie
                        satisfait ou remboursé. Si vous n'êtes pas satisfait,
                        contactez-nous.
                      </p>
                    </div>
                  </div>
