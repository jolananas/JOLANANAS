...
> **Fichier compromis** : `apps/frontend/.env.local`
...
5. **Installez l'app**
6. **Récupérez le Storefront Access Token** (commence par `shpat_` ou `shpca_`)
...
Si vous préférez mettre à jour manuellement, ouvrez `apps/frontend/.env.local` et remplacez :
