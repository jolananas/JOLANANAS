  ...
  useEffect(() => {
    ...
    if (!isSDKLoaded || disabled || !isValid) {
      if (process.env.NODE_ENV === 'development') {
        ...
        if (!isValid) {
          console.debug('🔍 PayPalButton: Props invalides', { checkoutId, amount, currency });
        }
      }
      return;
    }
    ...
    const buttonInstance = createPayPalButton({
      ...
      onSuccess: (orderId, transactionId) => {
        if (process.env.NODE_ENV === 'development') {
          console.log('✅ PayPalButton: Paiement réussi', { orderId, transactionId });
        }
        if (onSuccess) {
          onSuccess(orderId, transactionId);
        }
    ...
    if (state.status === 'error' && state.error) {
      ...
      if (!isCriticalError) {
        ...
        const timer = setTimeout(() => {
          if (process.env.NODE_ENV === 'development') {
            console.debug('🔄 PayPalButton: Réinitialisation automatique après erreur');
          }
          reset();
        }, 10000);
          ...
          <AlertDescription>
            ...
            <span className="text-xs mt-1 block">
              {state.error.includes('SDK') || state.error.includes('chargement') || state.error.includes('non disponible')
                ...
                : state.error.includes('Draft order') || state.error.includes('non trouvé')
      ...
      {state.status === 'success' && (
        <Alert className="mt-4 border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950">
          <AlertDescription className="text-green-800 dark:text-green-200">
            ✅ Paiement PayPal réussi ! Redirection en cours <LoadingDots size="sm" />
          </AlertDescription>
        </Alert>
      )}
    </div>
