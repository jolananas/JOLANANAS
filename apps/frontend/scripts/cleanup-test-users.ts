    ...
    for (const user of testUsers) {
      try {
        ...
        if (shopifyCustomerId) {
          ...
          console.log(
            `  ⚠️  Utilisateur ${user.email} n'a pas de shopifyCustomerId - nettoyage partiel`,
          );
        }
        ...
        await prisma.user
          ...
          .catch((err) => {
            console.warn(
              `  ⚠️  Impossible de supprimer l'utilisateur local ${user.email}:`,
              err,
            );
          });

        console.log(
          `  ✅ Nettoyé: ${user.email}${shopifyCustomerId ? ` (Shopify ID: ${shopifyCustomerId})` : " (pas de Shopify ID)"}`,
        );
      } catch (error) {
