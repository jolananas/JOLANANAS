...
export function CheckoutPage({ className, onComplete }: CheckoutPageProps) {
  ...
  const isFrance =
    shippingData.country === "FR" || shippingData.country === "France";
    ...
    try {
      ...
      const cartLines = items.map((item) => {
        ...
        if (
          ...
          console.error(
            "❌ Erreur: ID de produit utilisé au lieu d'un ID de variant:",
            variantId,
          );
          throw new Error(
            "Erreur: Le produit sélectionné nécessite de choisir une variante. Veuillez retourner à la page produit.",
          );
        }
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        if (problematicBefore.length > 0) {
          console.warn(
            "⚠️ Caractères Unicode détectés AVANT normalisation dans shippingInfo:",
          );
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        if (problematicAfter.length > 0) {
          console.error(
            "❌ ERREUR : Caractères Unicode toujours présents APRÈS normalisation dans normalizedShippingInfo:",
          );
          ...
          console.log(
            "✅ Aucun caractère Unicode problématique détecté dans normalizedShippingInfo après normalisation",
          );
        }
      }
      ...
      if (!response.ok) {
        ...
        const errorMessage =
          errorData.error || "Erreur lors de la création du panier";
      ...
      <div className="container py-8 md:py-12">
        ...
        <Card className="border-0 shadow-none bg-transparent mb-8">
          <CardHeader>
            ...
            <CardDescription>
              Remplissez vos informations de livraison, puis vous serez redirigé
              vers la page de paiement sécurisé.
            </CardDescription>
          </CardHeader>
        </Card>
            ...
            {currentStep === "shipping" && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Ces informations seront utilisées pour la livraison de votre
                    commande.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label
                        htmlFor="firstName"
                        className="text-jolananas-pink-medium"
                      >
                  ...
                  {isFrance ? (
                    <AddressAutocomplete
                      ...
                      onSelect={(suggestion) => {
                        ...
                        const normalizedStreet = normalizeDataForAPI(
                          suggestion.street,
                        );
                        ...
                        const normalizedHousenumber = normalizeDataForAPI(
                          suggestion.housenumber,
                        );
                        const normalizedPostcode = normalizeDataForAPI(
                          suggestion.postcode,
                        );
                        const normalizedDepartmentName = normalizeDataForAPI(
                          suggestion.departmentName,
                        );
                        const normalizedDepartment = normalizeDataForAPI(
                          suggestion.department,
                        );
                        const normalizedRegion = normalizeDataForAPI(
                          suggestion.region,
                        );
                        ...
                        if (
                          ...
                          cleanAddress = cleanAddress
                            .replace(
                              new RegExp(`\\s*${normalizedPostcode}\\s*`, "g"),
                              "",
                            )
                            .trim();
                        }
                        if (
                          ...
                          cleanAddress = cleanAddress
                            .replace(
                              new RegExp(`\\s*${normalizedCity}\\s*$`, "g"),
                              "",
                            )
                            .trim();
                        }
                        ...
                        setShippingData({
                          ...
                          postalCode:
                            normalizedPostcode || shippingData.postalCode,
                          department:
                  ...
                  <div className="space-y-2">
                    ...
                    {!addressHasNumber && isFrance && (
                      <p className="text-xs text-muted-foreground flex items-start gap-1.5 mt-1">
                        <span className="text-jolananas-pink-medium mt-0.5">
                          💡
                        </span>
                        <span>
                          ...
                          bâtiment, résidence, etc.) pour faciliter la
                          livraison.
                        </span>
                      </p>
                    )}
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    <div className="space-y-2 sm:col-span-1">
                      ...
                      {isFrance && (
                        <p className="text-xs text-muted-foreground">
                          Rempli automatiquement lors de la sélection d'une
                          adresse
                        </p>
                      )}
                    </div>
