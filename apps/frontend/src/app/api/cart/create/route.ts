...
export async function POST(request: NextRequest) {
  try {
    ...
    if (lines) {
      const isValidLineItem = lines.every(
        (line) =>
          line.merchandiseId &&
    ...
    if (shippingInfo && ENV.SHOPIFY_STOREFRONT_TOKEN) {
      try {
        ...
        if (existingCustomer) {
          ...
          const updatedCustomer = await adminClient.updateCustomer(
        ...
        console.error(
          "⚠️ Erreur lors de la création/mise à jour du client:",
          error,
        );
      }
    ...
    if (shippingInfo) {
      try {
        ...
        url.searchParams.set(
          ...
          normalizeForURL(shippingInfo.firstName),
        );
        url.searchParams.set(
          "checkout[shipping_address][last_name]",
          normalizeForURL(shippingInfo.lastName),
        );
        url.searchParams.set(
          "checkout[shipping_address][address1]",
          normalizeForURL(shippingInfo.address),
        );
        if (shippingInfo.address2) {
          url.searchParams.set(
            "checkout[shipping_address][address2]",
            normalizeForURL(shippingInfo.address2),
          );
        }
        url.searchParams.set(
          "checkout[shipping_address][city]",
          normalizeForURL(shippingInfo.city),
        );
        url.searchParams.set(
        ...
        console.error(
          "⚠️ Erreur lors de la construction de l'URL de checkout:",
          error,
        );
      }
    }
