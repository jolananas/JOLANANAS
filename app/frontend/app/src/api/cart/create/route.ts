...
export async function POST(request: NextRequest) {
  try {
    ...
    // Type guard pour la réponse cart
    const cartResponse = cartData.data as CartCreateResponse;

    // Vérifier les userErrors dans la réponse
    ...
    // Construire l'URL de checkout avec pré-remplissage si informations client disponibles
    let checkoutUrl = cart.checkoutUrl;
    if (shippingInfo) {
      try {
        const url = new URL(checkoutUrl);
        
        // Fonction helper pour normaliser les valeurs avant de les ajouter aux searchParams
        // Évite l'erreur ByteString avec les caractères Unicode
        // Utilise normalizeDataForAPI pour une normalisation complète
        const normalizeForURL = (value: string | undefined): string => {
          if (!value) return '';
          // Utiliser normalizeDataForAPI pour une normalisation complète
          const normalized = normalizeDataForAPI(value);
          // Vérification supplémentaire : s'assurer qu'il n'y a aucun caractère > 255
          const cleaned = typeof normalized === 'string' 
            ? normalized.split('').map(char => {
                const code = char.charCodeAt(0);
                // Si le caractère est > 255, le remplacer par un tiret simple
        ...
        // Pré-remplir les informations dans l'URL de checkout (valeurs normalisées)
        ...
        console.log('✅ URL de checkout pré-remplie avec les informations client');
      } catch (error) {
        console.error('⚠️ Erreur lors de la construction de l\'URL de checkout:', error);
        // Utiliser l'URL de checkout originale si la construction échoue
      }
    }
    ...
    console.log('✅ Panier créé:', formattedCart.id);
