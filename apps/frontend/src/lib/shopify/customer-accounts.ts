 ...
 * @param clientId - CLIENT_ID de Customer Account API (doit être exposé via * ou passé en paramètre)
...
export async function createCustomerAccessTokenFrontend(
  ...
  try {
    ...
    console.error(
      "❌ Erreur création token client (Customer Account API):",
      error,
    );
    return {
      accessToken: null,
...
export async function getCustomerFrontend(
  ...
  try {
    ...
    console.error(
      "❌ Erreur récupération client (Customer Account API):",
      error,
    );
    return {
      customer: null,
...
export async function createCustomerAccessToken(
  ...
  try {
    ...
    return {
      accessToken: null,
      errors: [
        { message: "Erreur lors de l'authentification", code: "AUTH_ERROR" },
      ],
    };
  }
}
...
export async function updateCustomerAddress(
  ...
  try {
    ...
    const addresses = (customerResponse.data.customer.addresses || []).map(
      (addr: any) => {
    ...
    if (!updatedAddress) {
      return {
        ...
        errors: [
          {
            message: "Adresse non trouvée après mise à jour",
